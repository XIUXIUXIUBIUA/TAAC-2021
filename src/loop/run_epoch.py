...
def training_loop(model, loader, loss_compute, epoch):
    model.train()
    losses = []
    ...
    for i, batch in enumerate(tqdm(loader, desc=f'train ({epoch})')):
        ...
        loss_dict = {}
        loss = 0
        for modal in (model.modal_name_list + ['fusion']):
            if(modal=='fusion'):
    ...
    return losses
def validating_loop(model, loader, loss_compute,epoch):
    model.eval()
    tagging_class_num = 82
    evl_metrics = [train_util.EvaluationMetrics(tagging_class_num, top_k=20)
                               for i in range(len(model.modal_name_list)+1)] #+1 for fusion
    ...
    for i,batch in enumerate(loader):
        ...
        for index,modal_name in enumerate(model.modal_name_list+['fusion']):
