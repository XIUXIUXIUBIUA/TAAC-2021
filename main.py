...
if __name__ == '__main__':
    ...
    train_loader = DataLoader(train_dataset,num_workers=10,batch_size=config['DatasetConfig']['batch_size'],shuffle=True,collate_fn=train_dataset.collate_fn)
    val_loader = DataLoader(val_dataset,num_workers=10,batch_size=config['DatasetConfig']['batch_size'],collate_fn=val_dataset.collate_fn)
    # 定义模型
    ...
    for epoch in range(epoch_num):
        ...
        if(epoch%5==0): # 每20个epoch 验证一次
            gap_dict = validating_loop(model,val_loader, loss_compute,epoch)
